...
// M03 Fase 6 — Writing Lab (docs/curriculum/M03.md "Fase 6"). Identical
// shape to seed-m02-phase6-writing.ts — LessonWritingLab.tsx is
// script-agnostic. No lesson_content_blocks to seed.
  ...
  try {
    ...
    const [phase] = await db
      ...
      .values({
        ...
        titleId: "Writing Lab",
        orderIndex: 20,
        descriptionId: "Latihan menulis Katakana mendalam — stroke mastery, kata, dan menulis buta.",
      })
      .onConflictDoUpdate({
    ...
    console.log(`Selesai. P20 id=${phase.id}, 3 lesson writing_lab (tanpa konten seed — data-driven dari pool).`);
  } finally {
    await close();
  }
}
...
main().catch((error) => {
  console.error("seed-m03-phase6-writing gagal:", error);
  process.exit(1);
});
