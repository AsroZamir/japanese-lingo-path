...
export function Avatar({ user }: { user: CurrentUser }) {
  if (user.avatarUrl) {
    // eslint-disable-next-line @next/next/no-img-element -- external Google avatar, no next.config.ts remotePatterns configured for next/image
    return <img src={user.avatarUrl} alt={user.name} referrerPolicy="no-referrer" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />;
  }
...
function AppShellContent({ user, children }: { user: CurrentUser; children: React.ReactNode }) {
